...
## Sommaire / Summary
- 🇫🇷 [Version Française](#-politique-de-confidentialité--izyprix-fr)
- 🇬🇧 [English Version](#-privacy-policy--izyprix-en)
...
- **Serveur local :** Communication avec l’IP locale (192.168.x.x) pour récupérer les informations produits et envoyer les impressions.  
- **Configuration dynamique :** Une requête unique vers un fichier texte sécurisé (GitHub Gist) permet de récupérer l’IP actuelle du serveur.  
  Aucune donnée personnelle n’est transmise.

---

## 2. Utilisation des données
Les données manipulées (codes-barres, noms, prix) servent uniquement à :

- identifier les articles,  
- afficher les informations produits,  
- imprimer des étiquettes de prix.

---

## 3. Stockage et Conservation
- **Aucune donnée personnelle collectée.**  
- **Données locales :** Seules les préférences d’affichage sont stockées dans la mémoire privée de l’appareil.  
...
## 5. Sécurité
Bien que l’application utilise HTTP pour la communication locale, l’accès est strictement limité au réseau privé du magasin ProAlim.
...
- **Local server:** The app communicates with the store’s local IP address (192.168.x.x) to retrieve product information and send print commands.  
...
## 2. Use of Data
The data handled (barcodes, product names, prices) is used exclusively to:
...
- **Local data:** Only display preferences are stored in the app’s private storage.  
...
## 5. Security
Although the app uses HTTP for local communication, access is strictly limited to the private ProAlim network.
